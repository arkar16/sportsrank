...
def html_grab(start_year, end_year, division):
    ...
    title_html += "</body>\n"
    title_html += "</html>\n"
    html_nc = f'<a href="cfb/years/history/nc_{division}_CFB_output.html"><h1>Link to all-time {division} CFB National Champions</h1></a><br>\n' # TODO CFB to {SPORT}
    html_wt = f'<a href="cfb/years/history/wt_{division}_CFB_output.html"><h1>Link to all-time {division} CFB Worst Teams</h1></a><br>\n' # TODO CFB to {SPORT}
    ...
    for year in range(end_year, start_year - 1, -1): # FIXME links are broken from here down
        html_template = f'<a href="sportsrank/cfb/years/{year}/rankings/{year}_FINAL_{division}_cors.html">Link to {year} {division} CFB final rankings</a><br>\n'
