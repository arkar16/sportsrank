...
for team in fbs_teams:
    logo = team.logos[0]
    logo_png = f"<img src='{logo}' style='width: 25px; height: 25px;'>"
    school = team.school
    conf = team.conference

    # Add the data to the dataframe
    cfb_teams = pd.concat([cfb_teams, pd.DataFrame({"logo": logo_png, "school": school, "conference": conf}, index=[0])], ignore_index=True)
